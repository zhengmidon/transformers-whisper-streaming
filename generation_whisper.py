...
class WhisperGenerationMixin(GenerationMixin):
    def _extract_token_timestamps(
        ...
        for batch_idx in range(batch_size):
            if num_frames is not None and isinstance(num_frames, (tuple, list, np.ndarray, torch.Tensor)):
                ...
                #ee=time.time()
                #print(f"\nmedian filter cost time {ee - ss:.3f}s")
    ...
    def generate(
        ...
        **kwargs,
        ...
        #print(f"generation_config after init {generation_config}")
        ...
        #print(f"generation_config after self._set_return_outputs {generation_config}")
        ...
        #print(f"generation_config after self._set_return_timestamps {generation_config}")
        self._set_language_and_task(
            language=language, task=task, is_multilingual=is_multilingual, generation_config=generation_config
        )
        #print(f"generation_config after self._set_language_and_task {generation_config}")
        self._set_num_frames(
            return_token_timestamps=return_token_timestamps, generation_config=generation_config, kwargs=kwargs
        )
        #print(f"generation_config after self._set_num_frames {generation_config}")
        ...
        #print(f"generation_config after self._set_thresholds_and_condition {generation_config}")
        ...
        while (seek < max_frames).any():
            ...
            #kwargs['past_key_values'] =torch.tensor([], device='cuda:0')
            #print(f"generation_config before generate {generation_config}")
            (
                seek_sequences,
                seek_outputs,
                should_skip,
            ...
            #print(f"input_features {input_features}, shape {input_features.shape}")
            #print(f"segment_input {segment_input}, shape {segment_input.shape}")
            #print(f"kwargs {kwargs}")
            #print(f"decoder_input_ids {decoder_input_ids}, shape {decoder_input_ids.shape}")
            #print(f"cur_bsz {cur_bsz}")
            #print(f"batch_idx_map {batch_idx_map}")
            #print(f"seek {seek}")
            #print(f"num_segment_frames {num_segment_frames}")
            #print(f"max_frames {max_frames}")
            #print(f'temperatures {temperatures}')
            #print(f"logits_processor {logits_processor}")
            #print(f"stopping_criteria {stopping_criteria}")
            #print(f"prefix_allowed_tokens_fn {prefix_allowed_tokens_fn}")
            #print(f"synced_gpus {synced_gpus}")
            #print(f"batch_size {batch_size}")
            #print(f'do_condition_on_prev_tokens {do_condition_on_prev_tokens}')
    ...
    def generate_with_fallback(
        ...
        for fallback_idx, temperature in enumerate(temperatures):
            ...
            #print(f"past_key_values {seek_outputs.past_key_values}")
            #print(f"seek_outputs {type(seek_outputs)}")
            ...
            # e=time.time()
            # print(f"\n post process cost time {e - s:.3f}s")
            #print(f"seek_outputs type afterpostprocess {type(seek_outputs)}")
    ...
    def _postprocess_outputs(
        ...
        if return_token_timestamps and hasattr(generation_config, "alignment_heads"):
            ...
            # e = time.time()
            # print(f"\nextract_token_timestamps cost time {e - s:.3f}s")
